    ...
    <div class="item-group" v-for="(item,index) in list" :key="item.id">
      <div class="head">
        ...
        <ButtonGroup style="float:right;margin-top:10px" size="small" v-if="item.applyStatus != 4">
          <Button type="primary" @click="act(item,1)">开始处理</Button>
          <Button type="primary" @click="act(item,1)">处理完成</Button>
          <Button type="primary" @click="act(item,1)">取消处理</Button>
          <Button type="primary" @click="act(item,1)">删除此条</Button>
        </ButtonGroup>
      </div>
