<template>
  <div class="panel">
    <Card :bordered="false" class="card">
      <p slot="title">合同费用种类信息配置</p>
      <Button slot="extra" size="small" type="success" @click="openModel(false)">新增费用种类</Button>
      <div class="card-contnet">
      <div class="table-contnet">
        <Row class-name="head">
            <Col class-name="col" span="7">费用种类名称</Col>
            <Col class-name="col" span="5">费用种类备注</Col>
            <Col class-name="col" span="5">操作人</Col>
    ...
    <Modal v-model="show" :title="isEdit? '编辑费用种类' : '新增费用种类'" :closable="false" :mask-closable="false">
      <Form :label-width="80" :ref="ref" :model="itemApi" :rules="rules">
          <FormItem label="费用种类" prop="name">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$http.post(this.api.queryBaseCostList,this.apiData).then(res =>{
        if(res.code === 1000){
    ...
    handleSubmit() {
      this.$refs[this.ref].validate((valid) => {
          if(valid){
              ...
              let apiUrl = this.isEdit ? this.api.saveAndUpdateBaseCost : this.api.saveAndUpdateBaseCost;
    ...
    deleteItem(item) {
        this.$Modal.confirm({
            ...
            onOk: () =>{
              ...
              this.$http.post(this.api.deleteBaseCost, params).then(res => {
                  if(res.code === 1000){
                      this.getList();
