    ...
    <div>
        <div class="filters">
            <Form ref="formInline" inline :label-width="120">
                <FormItem label="卖家公司名称：" class="magin0" style="width: 280px;">
                    <Input type="text" v-model="detail.sellName" placeholder="请输入..."></Input>
                </FormItem>
                ...
                <FormItem label="卖家手机号：" class="magin0" style="width: 280px;">
        ...
        <div class="card-contnet">
            <div class="table-contnet">
                <Row class-name="head">
                    <Col class-name="col" span="6">卖家公司名称</Col>
                ...
                <Row v-for="(item,index) in list" :key="item.sellId">
                    <Col class-name="col" span="6">{{item.sellName}}</Col>
                    ...
                    <Col class-name="col" span="4">
                        <template v-if="item.contractStatus == 1">
                            <Button type="warning" @click="goRouter(item)" size="small">选择卖家</Button>
                        </template>
                        <template v-else>
                            暂未认证
                        </template>
...
import {authStatus} from '@/utils/filters.js'
    export default {
        ...
        methods: {
            ...
            getAllList() {
                this.$http.post(this.api.queryBgSellContractMangerPage, this.handleData).then(res => {
                    if (res.code === 1000) {
            ...
            goRouter(data) {
                this.$router.push('step3-'+this.buyId+'-'+ data.sellId)
            }
        },
        created() {
            this.getAllList()
