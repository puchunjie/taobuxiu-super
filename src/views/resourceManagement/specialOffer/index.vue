  ...
  <div>
    <div class="filter-container">
      <div class="content">
        ...
        <div class="kp-card" v-for="item in list" :key="item.resourceId">
             <div class="head">
                资源编号：{{ item.resourceId }}
                <span>负责专员：{{ item.salemanName }}</span>
                <div class="action">
                    <template v-if="item.status == '1'">
                        <a @click="optionBtn(item.resourceId,1)">更新资源</a>
                        <a @click="optionBtn(item.resourceId,2)">立即下架</a> 
                        <a @click="optionBtn(item.resourceId,3)">删除资源</a>
                    </template>
                    <template v-else-if="item.status == '0' || item.status == '2' || item.status == '3'">
                        <a @click="optionBtn(item.resourceId,4)">立即上架</a>
                        <a @click="optionBtn(item.resourceId,3)">删除资源</a>
                    </template>
