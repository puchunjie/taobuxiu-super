  ...
  <div class="panel">
    <Card :bordered="false" class="card">
      ...
      <Form ref="formValidate" :model="dataApi" :rules="ruleValidate" :label-width="100" inline class="addForm">
        ...
        <FormItem label="服务支持：">
          <!-- <CheckboxGroup v-model="dataApi.auctionRights">
              <Checkbox v-for="(item,index) in baseData[7].list" :key="index" :label="`${item.name}`">
                <span>{{item.name}}</span>
              </Checkbox>
            </CheckboxGroup> -->
          <div class="item-content clearfix">
            <div class="item" v-for="(item,index) in baseData[7].list" :key="index" @click="selectItem(item,index)">
              <span class="iconfont" :class="item.isCheck ? 'icon-check-box' : 'icon-check_box_unselecte'"></span>
              {{ item.name }}
            </div>
          </div>
        </FormItem>
  ...
  export default {
    ...
    methods: {
      ...
      selectItem(item, index) {
        if (item.isCheck) {
          this.dataApi.rights.splice(index,1)
        } else {
