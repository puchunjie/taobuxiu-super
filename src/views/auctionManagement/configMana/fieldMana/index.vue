<template>
  <div class="panel">
    <Card :bordered="false" class="card">
      <p slot="title">场次分类管理</p>
      <Button slot="extra" size="small" type="success" @click="openModel(false)">新增场次分类</Button>
      <div class="card-contnet">
        <div class="table-contnet">
          <Row class-name="head">
            <Col class-name="col" span="6">分类名称</Col>
            <Col class-name="col" span="6">分类备注</Col>
            <Col class-name="col" span="6">分类排序</Col>
...
<script>
  export default {
    ...
    methods: {
      getList() {
        this.$http.post(this.api.findAuctionIndexList, this.pageApi).then(res => {
          if (res.code === 1000) {
      ...
      deleteItem(data) {
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            this.$http.post(this.api.deleteAuctionIndex, params).then(res => {
              if (res.code === 1000) {
                this.getList()
      ...
      handleSubmit() {
        this.$refs[this.ref].validate((valid) => {
          if (valid) {
            ...
            let apiUrl = this.isEdit ? this.api.updateAuctionIndex : this.api.saveAuctionIndex;
